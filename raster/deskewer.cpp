...
double 
Deskewer::findSkew(const QImage &img){
    unsigned int w2=next_pow2((img.width()+7)/8);
...
void 
Deskewer::radon(const QImage &img, int sign, unsigned int sharpness[]){
    unsigned short int *p1_, *p2_; // Stored columnwise
    
    unsigned int w2=next_pow2((img.width()+7)/8);
    unsigned int w=(img.width()+7)/8;
    unsigned int h=img.height();
